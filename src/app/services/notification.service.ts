...
export class NotificationService {
  ...
  public handleHttpError(error: any): void {
    ...
    if (!error) {
      ...
      this.error(this.httpErrorPath, "Ein unbekannter Fehler ist aufgetreten");
    }
    else if (error.status === 400) {
      console.info('[BaseCommunicationService] - Bad request was sent to backend');
      this.error(this.httpErrorPath, "Die Anfrage hat nicht geklappt");
      ...
      this.error(this.httpErrorPath, `Es ist ein Fehler aufgetreten (Code: ${error?.status})`);
    }
  }
